...
open class Menu {
    ...
    private val openSound = mutableListOf<String>()

    private val closeSound = mutableListOf<String>()

    private val buttons = mutableMapOf<String, Pair<ItemStack, List<String>>>()

    private fun loadLayout(menuType: String) {
        slots.clear()
        layout.clear()
        val regex = Regex("`[A-Za-z0-9_-]+`")
        Lycoris.menuConfig.getStringList("menu-${menuType}.layout").also {
    ...
    private fun loadButtons(menuType: String) {
        buttons.clear()
        Lycoris.menuConfig.getConfigurationSection("menu-${menuType}.buttons")?.let { config ->
    ...
    fun loadMenu(menuType: String) {
        title = Lycoris.menuConfig.getStringColored("menu-${menuType}.title") ?: title
        loadLayout(menuType)
        loadButtons(menuType)
    }

    protected fun openMenu(sender: Player, menuType: String, elements: () -> List<Title>, onClick: (ClickEvent, Title) -> Unit) {
        buildMenu<Linked<Title>>(title) {
            ...
            onGenerate { player, element, index, slot ->
                element.getIcon(sender, player, menuType)
            }
            onClick { e, element ->
                onClick(e, element)
            }
