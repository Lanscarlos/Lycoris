package top.lanscarlos.aiurtitle.module.config
...
object TitleLoader {
    ...
    private val folder by lazy {
        val folder = File(AiurTitle.plugin.dataFolder, "Titles")
    ...
    fun loadTitles() {
        ...
        mutableListOf<YamlConfiguration>().apply {
            ...
            it.getKeys(false).forEach { key ->
                ...
                AiurTitle.debug(2, "正在载入称号 $key ...")
                TitleData.loadTitle(key, it.getConfigurationSection(key))?.let { title ->
                    count += 1
                    AiurTitle.debug(1, "载入称号成功 $key -> ${title.getDisplay()}")
                    return@forEach
                }
                AiurTitle.debug(2, "载入称号 $key 失败")
            }
        }
